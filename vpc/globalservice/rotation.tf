# ──────────────────────────────────────────────────────────────────────────────
# Secrets Manager 로테이션 Lambda — service RDS PostgreSQL
# VPC1은 IGW/NAT 경유로 Secrets Manager 접근 가능
# ──────────────────────────────────────────────────────────────────────────────

resource "aws_security_group" "rotation_lambda" {
  name        = "financial-vpc1-rotation-lambda-sg"
  description = "Secrets Manager rotation Lambda for service RDS"
  vpc_id      = aws_vpc.this.id

  egress {
    description     = "Allow PostgreSQL to service RDS"
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
  ...
  egress {
    description = "Allow HTTPS outbound (Secrets Manager via NAT)"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
...
resource "aws_serverlessapplicationrepository_cloudformation_stack" "service_rds_rotation" {
  name             = "financial-service-rds-rotation"
  ...
  parameters = {
    ...
    functionName        = "financial-service-rds-rotation"
...
data "aws_lambda_function" "service_rotation" {
  function_name = "financial-service-rds-rotation"
