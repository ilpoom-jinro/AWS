...
| VPC | EKS | 배포 대상 |
|---|---|---|
| 서비스 VPC | `financial-service-eks` | Frontend, Backend, X-Ray Collector, Alloy |
...
| [x] | X-Ray Collector Kubernetes YAML에 ECR 이미지 URI 반영 | `financial-service-eks/xray/xray-collector.yaml` |
...
| [ ] | X-Ray Collector | `financial-service-eks/xray/xray-collector.yaml` | 서비스 EKS 배포용 Application 작성 |
...
`helmChart/alloy-service/values.yaml`의 자리표시자 두 개를 실제 DNS로 바꿉니다.
...
| [ ] | X-Ray Collector IAM Role 연동 확인 | Pod Identity Terraform 코드 작성 완료. 적용 후 확인 |
