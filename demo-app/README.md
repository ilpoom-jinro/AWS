...
백엔드는 OpenTelemetry로 trace를 만들고 OTLP HTTP로 Collector 또는 Alloy에 전송합니다.
현재 Kubernetes GitOps 구성은 Grafana Tempo를 tracing backend로 사용합니다.
...
Kubernetes 예시에서는 켜져 있고, Service VPC Alloy로 trace를 보냅니다.

```yaml
OTEL_TRACING_ENABLED: "true"
OTEL_SERVICE_NAME: stock-demo-api
OTEL_EXPORTER_OTLP_TRACES_ENDPOINT: http://observability-alloy-service.observability.svc.cluster.local:4318/v1/traces
OTEL_EXPORTER_OTLP_TRACES_PROTOCOL: http/protobuf
```

Tempo에서 기대할 수 있는 흐름은 다음과 같습니다.
...
Loki는 trace 저장소가 아니라 log 저장소입니다. 요청 흐름을 보려면 Grafana Tempo 같은 tracing backend가 필요합니다. Loki는 trace id가 포함된 로그를 검색하거나, trace와 로그를 연결하는 용도로 같이 쓰는 것이 일반적입니다.
...
트레이싱을 켠 Kubernetes 환경에서는 400/500/slow 요청도 Tempo에서 확인할 수 있습니다.
