...
- **high_latency 탐지** (팀원 기여): Istio P95 요청 지연(istio_request_duration_milliseconds)이 임계값(HIGH_LATENCY_P95_MS, 기본 1000ms) 초과 시 탐지. 상태 이상(CrashLoop/OOM 등)이 없는 Running 파드만 메트릭 기반 점검. ※ Alloy에 Istio 텔레메트리 스크랩이 추가되어야 동작.
- **scale_out HPA 연동** (v1.3): 대상 워크로드는 HPA가 replicas를 관리하므로, scale_out은 kubectl scale이 아닌 HPA maxReplicas 상향(patch)으로 실행. analyzer가 strategy_detail에 [SCALE_OUT via HPA] action=patch_hpa ... 형태로 인코딩해 Platform Core가 파싱.
- **CPU 폴백 수정** (v1.3): kube-state-metrics 부재 시 CPU 폴백을 cadvisor spec(container_spec_cpu_quota/period) 기반으로 변경해 0~1 비율 보장.
