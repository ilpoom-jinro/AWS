...
async def execute_remediation(plan: RemediationPlan) -> ExecutionResult:
    ...
    try:
        if plan.strategy == "restart":
            ...
            namespace = params.get("namespace", "default")
            # maxReplicas+=2 → _parse_kv 결과: key='maxReplicas+', val='2'
            ...
            # output에 패치 전 값을 저장 → AIOps Workflow가 RemediationPlan.previous_hpa_max_replicas에 설정
            return ExecutionResult(
                workflow_id=plan.workflow_id,
                success=True,
                action_taken=(
    ...
    try:
        if plan.strategy == "scale_out":
            ...
            if plan.previous_hpa_max_replicas > 0:
                # 정확한 원복: execute_remediation이 기록한 패치 전 값 사용
            ...
            # restart / rollback strategy → rollout undo (pod 추론)
            namespace = params.get("namespace", "default")
            pod_name = _resolve_pod_name(params, plan)
