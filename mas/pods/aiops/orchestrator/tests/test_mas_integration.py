...
# 13. find_high_latency_pod — Istio P95 임계값 초과 최악 파드 선택 (ms)
async def _test_high_latency():
    mc = MetricsCollector("http://thanos:9090")
    series = [
        ({"pod": "fast-1"}, 200.0),
        ({"pod": "slow-1"}, 1500.0),
        ({"pod": "slowest-1"}, 2300.0),
        ({"pod": "nan-1"}, float("nan")),   # NaN → 제외
    ]
    with patch.object(mc, "_query_vector", AsyncMock(return_value=series)):
        hit = await mc.find_high_latency_pod("financial-ops-eks", "service-apps", 1000.0)
    assert hit == ("slowest-1", 2300.0)     # 임계값 초과 중 최악 1건
    # 전부 임계값 이하 → None (탐지 스킵)
    with patch.object(mc, "_query_vector", AsyncMock(return_value=[({"pod": "a"}, 300.0)])):
        none_hit = await mc.find_high_latency_pod("c", "n", 1000.0)
