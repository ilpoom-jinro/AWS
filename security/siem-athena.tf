...
#
# 대상 소스:
#   · CloudTrail  — ilpumjinro-cloudtrail-logs-locked-v5  (JSON.gz, 멀티리전)
...
#
# 소스: s3://ilpumjinro-cloudtrail-logs-locked-v5/
...
resource "aws_glue_catalog_table" "cloudtrail" {
  ...
  parameters = {
    ...
    "storage.location.template" = "s3://ilpumjinro-cloudtrail-logs-locked-v5/AWSLogs/${var.account_id}/CloudTrail/$${region}/$${year}/$${month}/$${day}/"
  ...
  storage_descriptor {
    ...
    location = "s3://ilpumjinro-cloudtrail-logs-locked-v5/AWSLogs/${var.account_id}/CloudTrail/"
...
resource "aws_iam_role_policy" "siem_athena_query" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        # S3 원본 로그 버킷 3개 읽기
        # · CloudTrail: ilpumjinro-cloudtrail-logs-locked-v5 (Object Lock + key-cloudtrail 암호화)
